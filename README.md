...
- **Destination** — a "mini" bootstrap: asks one short question on first contact with a target that has no `.acm/destination.md` yet, escalating to a deeper multi-question interview when the stakes call for it — and can re-trigger *itself* when its own evidence says the destination is exhausted or wrong, while the operator always owns what the destination becomes.
- **Orient** — a one-time bootstrap on first contact producing a rubric and todo list, even with no trail history yet, plus a condensed arc-read the skill triggers itself thereafter when the work signals a need (contradictions, pushback, a stretch closing, staleness), with an entry-count backstop catching what judgment misses.
