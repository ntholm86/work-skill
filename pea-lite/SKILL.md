...
- **Material divergence** (the literal request and the likely actual goal point different directions, or the request is underspecified — "continue", "keep going", "next"): stop and narrate explicitly, before acting. State what you believe the destination is and what would count as success, and name at least one alternative reading you considered and rejected, and why — if you can't name one, you likely pattern-matched rather than interpreted. If the ask is bare-underspecified, form 1-3 sourced hunches from `.acm/destination.md`, `.acm/orientation.md`, and the last few trail entries about what matters most now, surface the single most useful question, and proceed on your best hunch as an explicit assumption if no answer comes back.
...
- **Tier 2 — Standard** (a real choice existed between plausible alternatives, or the work could plausibly be second-guessed later). Compact: interpretation (one line, only if it wasn't obvious), decision + one-line prediction, outcome vs. prediction, one line naming the one blind spot or nothing-found, and one line naming the single most obvious next-step candidate, if one stands out (skip if none does — silence is valid here too). Skip the full 4-trigger reflection ceremony unless one of the triggers below actually fires.
...
Use `[!DECISION]` (a genuine choice was made between plausible alternatives — the moment a future audit would want to find first), `[!REALIZATION]` (something shifted in what this run understood about the target that wasn't known going in), and `[!REVERSAL]` (an attempted approach was backed out of after it turned out wrong) at whatever tier — these markers are what makes the trail searchable and are never cut for cost, even in a Tier 1 entry, if one genuinely applies.
