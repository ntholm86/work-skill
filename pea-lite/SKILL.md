...
- If neither exists: this is a first run against this target. Do the condensed version of Destination:
  ...
  6. **Do not build other committed files around an unconfirmed guess in the same pass.** Labeling a guess `unconfirmed` in `destination.md` does not license writing the rest of the target's files as though it were settled. If the work ahead would produce other committed artifacts whose content depends on a step 0 or step 1 inference, ask before producing those artifacts too — not just before writing `destination.md` itself. This is not a hypothetical: `pea-lite`'s own bootstrap did exactly this (built `README.md` and design framing around three unconfirmed guesses before asking, then needed three separate corrections), found by an Orient run on 2026-07-03.
