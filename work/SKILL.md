...
- If `.acm/destination.md` exists (or the legacy name `.acm/vision.md`): read it, plus `.acm/orientation.md` if present. Continue to step 0.5.
...
3. Record this as a dated "Bootstrap" section in `.acm/orientation.md`, distinct from a later mini-orient's "Current claims" — there is no arc yet to claim anything about.

This runs once per target's first contact with Tier 2 or 3 work. After it, `.acm/orientation.md` exists and step 4's usual need-signals and backstop govern every future arc-read — this step does not repeat.
...
This is the *arc-read* mode of Orient — distinct from step 0.5's one-time bootstrap, which already produced the first rubric and todo list on first contact. The skill decides when a further arc-read is due — never wait for the operator to ask for one. Check at trail-append time, and run the mini-orient **in this same run** when any of these fires:
